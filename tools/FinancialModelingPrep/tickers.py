...
class Ticker(AbstractAPI):
    ...
    def analyst_estimates(self):
        """Get the analyst estiamtes for a particular company"""
        url = "analyst-estiamtes"
        res = self._get_data(url=url, ticker=",".join(self.tickers))
        # TODO transform into df
        return res
    
    @classmethod
    def get_analyst_estimates(cls, ticker: str):
        return cls(ticker=ticker).analyst_estimates()
